...
class PCE_NMPC:
    ...
    def NLP_specification(self):
        ...
        NV  = (NXD+NXA)*ns_N + NU + NUM    
        
        # NLP variable vector
        V   = MX.sym("V",NV+ng+ngp*nk+(nun+nd)**2*nk*ns_N+1)
        con = MX.sym("con",ns_N*(nd+nun)+nk+nu)
    ...
    def set_constraints(self):
        ...
        ffcn, D, v, ucl      = self.ffcn, self.D, self.v, self.ucl
        nd, na, nu, nx       = self.nd, self.na, self.nu, self.nd+self.na
        ...
        ufcn     = Function('ufcn',[va,yreala,ymeana,ucla],[va + clfcn(yreala,ymeana,ucla)])
        u1       = SX.sym('u1',nu)
        u2       = SX.sym('u2',nu)
        ...
        for k in range(nk):
            for l in range(ns_N):
                if k == 0:
                    ...
                    ymean = Meany[k,:]
                    g += [udifffcn(U[k][l],ufcn(v[k],yreal,ymean,ucl[0]))]
    ...
    t_pasts, xu_pasts, xuS_pasts, timeNMPC, timePCEs   
    ...
    def update_inputs(self,xu_N,tk,u_nmpc):
        nd, nk, nu, nun, ns_N = self.nd, self.nk, self.nu, self.nun, self.ns_N
        tk               += 1
        p                 = np.zeros(ns_N*(nd+nun)+nk+nu)
